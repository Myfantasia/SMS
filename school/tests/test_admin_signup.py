...
from apps.identity.models import AdminExtra, AdminInviteCode
...
class AdminSignupSecurityTests(TestCase):
    """
    Security/loophole pass on the admin-signup JSON API (school/views/public_api_views.py
    api_signup_admin / api_login_admin), wrapping the same form/business logic the
    original template-rendering admin_signup_view/admin_login_view used.
    """

    def test_bootstrap_admin_gets_immediate_access_without_invite_code(self):
        response = self.client.post(reverse('api_public_signup_admin'), _valid_payload())
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['status'], 'success')
        self.assertTrue(response.json()['is_bootstrap'])
    ...
    def test_second_admin_without_invite_code_is_rejected(self):
        ...
        self.client.post(reverse('api_public_signup_admin'), _valid_payload())
        ...
        response = self.client.post(reverse('api_public_signup_admin'), _valid_payload(username='bob_admin', email='bob@example.com'))
        self.assertFalse(User.objects.filter(email='bob@example.com').exists())
        self.assertIn('Invalid or expired invite code', response.json()['message'])

    def test_second_admin_with_wrong_invite_code_is_rejected(self):
        self.client.post(reverse('api_public_signup_admin'), _valid_payload())
        response = self.client.post(
            reverse('api_public_signup_admin'),
    ...
    def test_second_admin_with_correct_invite_code_is_pending_not_active(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['status'], 'success')
        self.assertFalse(response.json()['is_bootstrap'])
    ...
    def test_duplicate_email_rejected(self):
        ...
        self.assertIn('already exists', str(field_errors['email']))
    ...
    def test_duplicate_username_rejected(self):
        ...
        self.client.post(reverse('api_public_signup_admin'), _valid_payload(username='ada_admin', email='ada@example.com'))
        code = _make_invite_code()
        response = self.client.post(reverse('api_public_signup_admin'), _valid_payload(
        ...
        self.assertIn('already taken', str(field_errors['username']))

    def test_password_confirmation_mismatch_rejected(self):
        response = self.client.post(reverse('api_public_signup_admin'), _valid_payload(password2='different-password'))
        self.assertFalse(User.objects.filter(email='ada@example.com').exists())
        field_errors = response.json()['field_errors']
        self.assertIn('password2', field_errors)
        self.assertIn('do not match', str(field_errors['password2']))

    def test_password_below_minimum_length_rejected(self):
        response = self.client.post(reverse('api_public_signup_admin'), _valid_payload(password='abc', password2='abc'))
        self.assertFalse(User.objects.filter(email='ada@example.com').exists())
        field_errors = response.json()['field_errors']
        self.assertIn('password', field_errors)
        self.assertIn('at least 8 characters', str(field_errors['password']))
    ...
    def test_weak_common_password_rejected(self):
        ...
        # a bare length check.
        response = self.client.post(reverse('api_public_signup_admin'), _valid_payload(password='password123', password2='password123'))
        self.assertFalse(User.objects.filter(email='ada@example.com').exists())
        self.assertIn('password', response.json()['field_errors'])

    def test_missing_required_fields_rejected(self):
        response = self.client.post(reverse('api_public_signup_admin'), _valid_payload(mobile='', address=''))
        self.assertFalse(User.objects.filter(email='ada@example.com').exists())
        self.assertEqual(response.status_code, 400)  # JSON field errors, no user created
    ...
    def test_full_two_step_approval_flow_grants_access(self):
        ...
        # Now bob can actually log in and gets routed to the admin dashboard (see
        # _resolve_post_login_destination in public_api_views.py).
        login_response = self.client.post(reverse('api_public_login_admin'), {
            'email': 'bob@example.com', 'password': 'correct-horse',
        })
        self.assertEqual(login_response.status_code, 200)
        login_data = login_response.json()
        self.assertEqual(login_data['status'], 'success')
        self.assertEqual(login_data['destination'], 'dashboard')
        self.assertEqual(login_data['path'], '/admin-dashboard')
...
class AdminVerificationCodeSecurityTests(TestCase):
    ...
    def test_wrong_code_against_hash_is_rejected_at_login(self):
        ...
        self.assertIn('Incorrect verification code', response.json()['message'])
