...
export default function StudentAssignments() {
  ...
  const actionFor = (item: BoardItem) => {
    if (item.is_locked && item.student_status === 'Not Started') {
      return <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400"><Lock className="w-3.5 h-3.5" /> Locked</span>;
    }
    if (item.student_status === 'Published') {
      return (
        <button onClick={() => navigate(`${item.id}/review`)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
          <Eye className="w-3.5 h-3.5" /> View Feedback
        </button>
      );
    }
    if (item.student_status === 'Returned') {
      if (item.is_locked) return <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400"><Lock className="w-3.5 h-3.5" /> Locked</span>;
      return (
    ...
    if (item.student_status === 'Pending') {
      ...
      if (canResubmit) {
        return (
          <button onClick={() => navigate(`${item.id}/take`)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
            <RotateCcw className="w-3.5 h-3.5" /> Resubmit
          </button>
        );
      }
      return <span className="text-xs text-slate-400 font-medium">Awaiting grading</span>;
    }
    // Not Started
    return (
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800">My Assignments</h1>
          <p className="text-sm text-slate-500 mt-0.5">Tasks and quizzes assigned to you.</p>
        </div>
      </div>
