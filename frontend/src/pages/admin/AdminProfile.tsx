...
export default function AdminProfile() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
          ...
          <form onSubmit={handlePasswordChange} className="space-y-5 max-w-md">
            ...
            <div>
              ...
              <PasswordInput
                ...
                className="pl-10 w-full p-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500 dark:focus:border-blue-400"
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full bg-slate-800 hover:bg-slate-900 text-white font-medium py-2.5 px-4 rounded-lg transition-colors disabled:bg-slate-400 dark:disabled:bg-slate-700"
            >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm dark:shadow-none border border-slate-100 dark:border-slate-700 p-6">
        <div className="flex items-center gap-2 mb-6 border-b border-slate-50 dark:border-slate-800 pb-4">
          <Users className="w-5 h-5 text-slate-400 dark:text-slate-500" />
          <div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Reset a User's Password</h3>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Applies to students, teachers, parents and staff. Admin accounts can only reset their own password.</p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl items-end">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">User Type</label>
            <select
              value={resetUserType}
              onChange={(e) => setResetUserType(e.target.value)}
              className="w-full p-2.5 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-blue-500 dark:focus:border-blue-400 text-sm bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100"
            >
          ...
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1">Select User</label>
            <SearchableSelect
              options={userOptions}
              value={resetUserId}
        ...
        <button
          ...
          className="mt-4 flex items-center gap-2 px-4 py-2.5 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-sm font-semibold rounded-lg hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      {passwordReveal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl dark:shadow-none max-w-sm w-full p-6 text-center">
            <div className="w-14 h-14 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center mx-auto mb-4">
              <KeyRound className="w-7 h-7" />
            </div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">Password Reset</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
              This is shown only once. Relay it to <span className="font-semibold text-slate-700 dark:text-slate-200">{passwordReveal.username}</span> out-of-band — they'll be required to set their own new password immediately after logging in with it.
            </p>
            <div className="flex items-center justify-center gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-4 mb-5">
              <span className="text-xl font-extrabold tracking-wide text-slate-800 dark:text-slate-100 break-all">{passwordReveal.newPassword}</span>
              <button onClick={copyResetPassword} className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors shrink-0" title="Copy password">
                <Copy className="w-4 h-4 text-slate-500 dark:text-slate-400" />
              </button>
            </div>
            {passwordReveal.emailNotified && (
              <p className="flex items-center justify-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400 font-medium mb-5">
