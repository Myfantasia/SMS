...
const AVATAR_COLOR: Record<UserType, string> = {
  ...
  admins: 'bg-rose-100 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400',
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0">
        <p className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{label}</p>
        <p className="text-sm text-slate-800 dark:text-slate-100 font-medium break-words">{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm dark:shadow-none overflow-hidden">
      <div className="p-5 border-b border-slate-100 dark:border-slate-700 flex items-center gap-2">
        <h2 className="text-base font-bold text-slate-800 dark:text-slate-100 capitalize">Pending {userType}</h2>
        <span className="bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 text-xs font-bold px-2.5 py-1 rounded-full">{users.length}</span>
      </div>

      {userType === 'admins' && users.length > 0 && (
        <div className="px-5 py-3 bg-rose-50/60 dark:bg-rose-500/10 border-b border-rose-100 dark:border-rose-500/20 text-xs text-rose-700 dark:text-rose-400 flex items-start gap-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/70 transition-colors">
                  ...
                  <td className="py-3 px-5 text-slate-500 dark:text-slate-400 text-sm">
                    {userType === 'students' && (user.class || user.extra_info || <span className="text-slate-300 dark:text-slate-600 italic">N/A</span>)}
                    {userType === 'teachers' && (user.subjects || user.extra_info || <span className="text-slate-300 dark:text-slate-600 italic">None listed</span>)}
                    {userType === 'parents' && (user.children || user.extra_info || <span className="text-slate-300 dark:text-slate-600 italic">No children linked</span>)}
                    {userType === 'admins' && <span className="text-amber-600 dark:text-amber-400 font-medium">Awaiting review</span>}
                    {userType === 'staff' && (
                      <div className="flex flex-col">
                        <span>{user.job_title || <span className="text-slate-300 dark:text-slate-600 italic">Not specified</span>}</span>
                  ...
                  <td className="py-3 px-5 text-right">
                    <div className="flex items-center justify-end">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3.5 py-1.5 bg-blue-50 dark:bg-blue-500/10 hover:bg-blue-100 dark:hover:bg-blue-500/20 text-blue-700 dark:text-blue-400 rounded-lg text-xs font-bold transition-colors disabled:opacity-60 whitespace-nowrap"
                      >
      ...
      {reviewUser && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl dark:shadow-none max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white dark:bg-slate-900 flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700 z-10">
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">Application Review</h3>
              <button onClick={closeReview} title="Close" className="text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100 dark:border-slate-700">
                  {reviewData.profile_pic ? (
                    <img src={reviewData.profile_pic} alt={reviewData.name} className="w-14 h-14 rounded-full object-cover border-2 border-slate-100 dark:border-slate-700 shrink-0" />
                  ) : (
                  ...
                  <div className="min-w-0">
                    <h4 className="text-lg font-extrabold text-slate-800 dark:text-slate-100 truncate">{reviewData.name || reviewUser.name}</h4>
                    <p className="text-sm text-slate-400 dark:text-slate-500 capitalize">{userType.slice(0, -1)} applicant &middot; {reviewData.username}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  {userType === 'staff' && (
                    <>
                      ...
                      {reviewData.id_number && <DetailRow icon={<Hash className="w-4 h-4" />} label="National ID Number" value={reviewData.id_number} />}
                      <DetailRow icon={<ShieldCheck className="w-4 h-4" />} label="Requested Access Role" value={reviewData.requested_role || <span className="italic text-slate-400 dark:text-slate-500">None selected</span>} />
                    </>
                  )}
                </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {reviewData.father_name && (
                        <div className="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg p-3">
                          <p className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold uppercase">Father</p>
                          <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{reviewData.father_name}</p>
                          {reviewData.father_mobile && <p className="text-xs text-slate-500 dark:text-slate-400">{reviewData.father_mobile}</p>}
                        </div>
                      )}
                      {reviewData.mother_name && (
                        <div className="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg p-3">
                          <p className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold uppercase">Mother</p>
                          <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{reviewData.mother_name}</p>
                          {reviewData.mother_mobile && <p className="text-xs text-slate-500 dark:text-slate-400">{reviewData.mother_mobile}</p>}
                        </div>
                      )}
                      {reviewData.guardian_name && (
                        <div className="bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-lg p-3">
                          <p className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold uppercase">
                            Guardian{reviewData.guardian_relationship ? ` (${reviewData.guardian_relationship})` : ''}
                          </p>
                          <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{reviewData.guardian_name}</p>
                          {reviewData.guardian_mobile && <p className="text-xs text-slate-500 dark:text-slate-400">{reviewData.guardian_mobile}</p>}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
                    ...
                    <div className="space-y-2">
                      {reviewData.children_detail.map((c: { id: number; name: string; roll: string; class: string }) => (
                        <div key={c.id} className="flex items-center justify-between p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                          <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{c.name} <span className="text-slate-400 dark:text-slate-500 font-normal">({c.roll})</span></p>
                          <span className="text-xs text-slate-500 dark:text-slate-400">{c.class}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {userType === 'admins' && (
                  <div className="mt-6 px-4 py-3 bg-rose-50/60 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20 rounded-xl text-xs text-rose-700 dark:text-rose-400 flex items-start gap-2">
                    <ShieldCheck className="w-4 h-4 mt-0.5 shrink-0" />
                ...
                <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700 flex items-center justify-end gap-3">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-700 dark:text-red-400 rounded-xl text-sm font-bold transition-colors disabled:opacity-60"
                  >
      ...
      {codeReveal && (
        <CodeRevealModal
          ...
          description={
            <>Relay this code to <span className="font-semibold text-slate-700 dark:text-slate-200">{codeReveal.name}</span> so they can enter it on the admin login page and activate their account.</>
          }
