...
export default function ViewClass() {
    ...
    return (
      <div className="animate-in fade-in zoom-in-95 duration-200">
        <div className="max-w-7xl mx-auto px-6 pt-6">
          <button onClick={() => setShowEnrollments(false)} className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors w-max">
  ...
  if (showCurriculum) {
    return (
      <div className="animate-in fade-in zoom-in-95 duration-200">
        <div className="max-w-7xl mx-auto px-6 pt-6">
          <button onClick={() => setShowCurriculum(false)} className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-emerald-600 transition-colors w-max">
            <ArrowLeft className="w-4 h-4" /> Back to Class Profile
          </button>
        </div>
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <button onClick={() => navigate(`${basePath}/classes`)} className="flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors w-max">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-3xl font-black text-slate-800">{classData.name}</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2 mb-4">
              <ClipboardCheck className="w-5 h-5 text-amber-500" /> Class Snapshot
            </h3>
            <div className="grid grid-cols-3 gap-4">
               <div className="bg-amber-50 p-4 rounded-lg border border-amber-100 text-center">
                 ...
                 <p className="text-xs text-amber-600 font-medium mt-1 uppercase tracking-wider">Capacity Filled</p>
               </div>
               <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100 text-center">
                 <p className="text-2xl font-black text-emerald-700">{assignedTeachers.length}</p>
                 <p className="text-xs text-emerald-600 font-medium mt-1 uppercase tracking-wider">Subjects Staffed</p>
               </div>
               <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 text-center">
                 <p className="text-2xl font-black text-blue-700">
                   {classData.class_teacher !== 'Not Assigned' ? <Star className="w-6 h-6 fill-blue-700 mx-auto" /> : '—'}
                 </p>
                 <p className="text-xs text-blue-600 font-medium mt-1 uppercase tracking-wider">Homeroom Assigned</p>
               </div>
            </div>
          </div>
                ...
                {isAdmin && (
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="text-sm text-blue-600 font-medium hover:underline px-3 py-1.5 bg-blue-50 rounded-md transition"
                    >
                      Manage Enrollments
                    </button>
                    <button 
                      onClick={() => setShowCurriculum(true)} 
                      className="flex items-center gap-1.5 text-sm text-emerald-700 font-medium hover:underline px-3 py-1.5 bg-emerald-50 rounded-md transition"
                    >
                    ...
                    {classData.students && classData.students.length > 0 ? (
                      classData.students.map(student => (
                        <tr key={student.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3 font-medium text-slate-600">{student.roll}</td>
                      ...
                      <tr>
                        <td colSpan={3} className="px-4 py-8 text-center text-slate-400">No students enrolled in this class yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            {loadingTimetable ? (
              ...
              <div className="space-y-3">
                {todayLessons.map((lesson) => (
                  <div key={lesson.id} className="flex gap-3 items-start p-3 rounded-lg hover:bg-slate-50 transition border border-transparent hover:border-slate-100">
                    <div className="bg-indigo-100 text-indigo-700 px-2 py-1 rounded text-xs font-bold w-16 text-center shrink-0">{lesson.start_time}</div>
                    <div>
                      <p className="text-sm font-bold text-slate-800">{lesson.subject}</p>
                      <p className="text-xs text-slate-500 flex items-center gap-1"><Clock className="w-3 h-3"/> {lesson.teacher}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {isAdmin && (
              <button
                ...
                className="w-full mt-4 py-2 bg-slate-100 text-slate-600 font-medium text-sm rounded-lg hover:bg-slate-200 transition"
              >
                Manage Timetable
              </button>
            )}
            ...
            {loadingTeachers ? (
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto pr-1 -mr-1">
                {assignedTeachers.map((item, idx) => (
                   <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 hover:bg-slate-100/70 rounded-lg border border-slate-100 transition-colors">
                     <div className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-xs shrink-0">
                       {item.teacher.charAt(0).toUpperCase()}
                     </div>
                     <div className="min-w-0 flex-1">
                       <p className="font-bold text-slate-800 text-sm truncate">{item.subject}</p>
