      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {/* Event Type */}
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-slate-700 mb-1">Event Category</label>
              <select 
                value={formData.event_type}
                onChange={(e) => setFormData({...formData, event_type: e.target.value})}
                className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white text-slate-700"
              >
            ...
            {/* Visibility Toggle */}
            <div className="md:col-span-2 flex items-center gap-3 bg-blue-50 p-3 rounded-lg border border-blue-100">
              <input 
                type="checkbox" 
                id="is_active" 
              ...
              />
              <div>
                <label htmlFor="is_active" className="font-semibold text-slate-800 cursor-pointer select-none">
                  {initialData ? 'Keep Published' : 'Publish Immediately'}
                </label>
                <p className="text-xs text-slate-500">If unchecked, this event is saved as a draft and hidden from the public website.</p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 mt-2 border-t border-slate-200 flex justify-end gap-3">
            ...
            <button 
              ...
              className="px-5 py-2.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-sm disabled:opacity-70 flex items-center gap-2"
            >
