    ...
    return (
      <div className="space-y-6 max-w-4xl animate-pulse">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[1, 2].map((i) => <div key={i} className="h-48 bg-slate-200 rounded-2xl"></div>)}
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl space-y-4 shadow-sm">
          ...
          <div className="space-y-1">
            <label htmlFor="maxConsecutivePeriods" className="text-xs font-bold text-slate-600">Max Consecutive Teacher/Class Periods</label>
            <input id="maxConsecutivePeriods" type="number" min="1" max="8" required value={maxConsecutivePeriods} onChange={(e) => setMaxConsecutivePeriods(Number(e.target.value))} className="w-full border border-slate-300 rounded-lg p-2 bg-white text-sm font-semibold" />
          </div>

          <div className="space-y-1">
            <label htmlFor="maxDailySubjectFrequency" className="text-xs font-bold text-slate-600">Max Single Day Subject Repeat Cap</label>
            <input id="maxDailySubjectFrequency" type="number" min="1" max="4" required value={maxDailySubjectFrequency} onChange={(e) => setMaxDailySubjectFrequency(Number(e.target.value))} className="w-full border border-slate-300 rounded-lg p-2 bg-white text-sm font-semibold" />
          </div>

          <div className="space-y-1">
            <label htmlFor="minSubjectSpacingPeriods" className="text-xs font-bold text-slate-600">Minimum Subject Spacing Interval (Periods)</label>
            <input id="minSubjectSpacingPeriods" type="number" min="0" max="5" required value={minSubjectSpacingPeriods} onChange={(e) => setMinSubjectSpacingPeriods(Number(e.target.value))} className="w-full border border-slate-300 rounded-lg p-2 bg-white text-sm font-semibold" />
          </div>
          ...
          <label className="flex items-center gap-2 pt-2 cursor-pointer select-none">
            ...
            <span className="text-xs font-bold text-slate-700">Allow double periods for Remedial tracks</span>
          </label>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-200 p-5 rounded-xl space-y-4 shadow-sm">
          ...
          <div className="space-y-1">
            <label htmlFor="heavyDayThreshold" className="text-xs font-bold text-slate-600">Heavy Subject Daily Threshold (Periods)</label>
            <input id="heavyDayThreshold" aria-label="Heavy Subject Daily Threshold in periods" placeholder="e.g. 3" type="number" min="1" max="6" required value={heavyDayThreshold} onChange={(e) => setHeavyDayThreshold(Number(e.target.value))} className="w-full border border-slate-300 rounded-lg p-2 bg-white text-sm font-semibold" />
          </div>

          <div className="space-y-1">
            <label htmlFor="maxWeeklyLessons" className="text-xs font-bold text-slate-600">Max Weekly Lessons Policy Cap</label>
            <input id="maxWeeklyLessons" type="number" min="10" max="60" required value={maxWeeklyLessons} onChange={(e) => setMaxWeeklyLessons(Number(e.target.value))} className="w-full border border-slate-300 rounded-lg p-2 bg-white text-sm font-semibold" />
          </div>

          <div className="space-y-1">
            <label htmlFor="enforcementMode" className="text-xs font-bold text-slate-600">Scheduler Compliance Mode</label>
            <select id="enforcementMode" value={enforcementMode} onChange={(e) => setEnforcementMode(e.target.value as 'STRICT' | 'SOFT')} className="w-full border border-slate-300 rounded-lg p-2 bg-white text-sm font-bold text-slate-700">
          ...
          <label className="flex items-center gap-2 pt-2 cursor-pointer select-none">
            ...
            <span className="text-xs font-bold text-slate-700">Block back-to-back heavy student days</span>
          </label>
        </div>
