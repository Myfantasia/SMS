...
export default function MasterTimetableView({ activeTimetable, slots, onBack }: MasterTimetableViewProps) {
  ...
  if (loading) {
    return (
      <div className="p-6 max-w-full mx-auto space-y-4 animate-pulse">
        ...
        <div className="h-125 bg-slate-200 dark:bg-slate-800 rounded-2xl"></div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-full mx-auto space-y-4 animate-fade-in">
      ...
      {/* Header */}
      <div className="flex flex-wrap justify-between items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-none">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight flex items-center gap-2">
              <LayoutGrid className="w-5 h-5 text-slate-500 dark:text-slate-400" /> Whole-School Timetable
            </h1>
        ...
        <div className="relative w-64">
          ...
          <input
            ...
            className="w-full border border-slate-300 dark:border-slate-600 rounded-lg pl-9 pr-8 py-2 text-sm bg-slate-50 dark:bg-slate-800 outline-none focus:border-indigo-500 dark:focus:border-indigo-400 font-medium text-slate-700 dark:text-slate-200"
          />
          {streamSearch && (
            <button
      ...
      {/* Day Tabs */}
      <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex gap-1 border border-slate-200 dark:border-slate-700 shadow-inner w-fit">
        {daysWithSlots.map(day => (
          <button
            key={day}
            onClick={() => setActiveDay(day)}
            className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${activeDay === day ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'}`}
          >
      ...
      {filteredStreams.length === 0 ? (
        <div className="border border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-16 text-center text-slate-400 dark:text-slate-500 bg-white dark:bg-slate-900">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm dark:shadow-none overflow-auto custom-scrollbar max-h-[calc(100vh-280px)]">
          <table className="border-collapse text-xs w-full">
            <thead className="sticky top-0 z-20">
              <tr>
                <th rowSpan={2} className="sticky left-0 z-30 bg-slate-100 dark:bg-slate-800 border-r border-b border-slate-200 dark:border-slate-700 p-2 min-w-25 text-slate-500 dark:text-slate-400 font-black uppercase">Period</th>
                {gradeGroups.map(g => (
                  <th key={g.grade_id} colSpan={g.streams.length} className="bg-slate-100 dark:bg-slate-800 border-b border-r border-slate-200 dark:border-slate-700 p-2 text-slate-700 dark:text-slate-200 font-black uppercase tracking-wide text-center whitespace-nowrap">
                    {g.grade_name}
                  </th>
                ))}
              </tr>
              <tr>
                {filteredStreams.map(s => (
                  <th key={s.id} className="bg-slate-50 dark:bg-slate-800/60 border-b border-r border-slate-200 dark:border-slate-700 p-2 text-slate-600 dark:text-slate-300 font-bold text-center whitespace-nowrap min-w-30">
                    {s.name}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {uniqueTimes.map(timeStr => {
                ...
                if (slot.is_global) {
                  return (
                    <tr key={timeStr}>
                      <td className="sticky left-0 z-10 bg-slate-50 dark:bg-slate-800/60 border-r border-b border-slate-200 dark:border-slate-700 p-2 text-center font-mono font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap">
                        {timeStr}
                      </td>
                      <td colSpan={totalColumns} className="border-b border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-center font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider p-2">
                        {slot.global_label}
                      </td>
                    </tr>
                  );
                ...
                return (
                  <tr key={timeStr} className={slot.is_remedial ? 'bg-purple-50/30 dark:bg-purple-500/10' : ''}>
                    <td className="sticky left-0 z-10 bg-white dark:bg-slate-900 border-r border-b border-slate-200 dark:border-slate-700 p-2 text-center font-mono font-bold text-slate-500 dark:text-slate-400 whitespace-nowrap">
                      {timeStr}
                    </td>
                    {filteredStreams.map(stream => {
                      const cellLessons = lessonsByCell.get(`${stream.id}_${slot.id}`) || [];
                      return (
                        <td key={stream.id} className="border-r border-b border-slate-100 dark:border-slate-700 p-1.5 align-top min-w-30">
                          {cellLessons.length === 0 ? (
                            ...
                            <div className="bg-slate-800 dark:bg-slate-700 text-white rounded-md p-1.5 leading-tight">
                              <p className="font-bold truncate">{cellLessons.length} Subjects</p>
                              <p className="text-slate-300 dark:text-slate-300 truncate">{cellLessons.map(l => l.subject_name).join(', ')}</p>
                            </div>
                          )}
                        </td>
                      );
      ...
      {filteredStreams.length > 0 && uniqueTimes.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 px-2 text-[11px] font-medium text-slate-500 dark:text-slate-400">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-purple-100 dark:bg-purple-500/20 border border-purple-200 dark:border-purple-500/40 shrink-0"></span> Remedial period</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-slate-800 dark:bg-slate-700 shrink-0"></span> Multiple subjects (option block)</span>
          <span className="text-slate-400 dark:text-slate-500">Cell colors match a subject across every stream</span>
        </div>
      )}
    </div>
  );
