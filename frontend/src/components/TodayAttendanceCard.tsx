...
export default function TodayAttendanceCard() {
  ...
  return (
    <div className="bg-white rounded-xl w-full h-full p-4 border border-slate-100 shadow-sm flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-lg font-bold text-slate-700">Today's Attendance</h1>
        <Link to="/admin-dashboard/attendance" className="text-xs font-semibold text-blue-600 hover:text-blue-700 transition-colors">
          View Details →
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-400 gap-2 text-center px-4">
        ...
        <div className="flex-1 grid grid-cols-2 gap-4">
          <div className="bg-emerald-50 rounded-xl p-4 flex flex-col justify-center">
            ...
            <span className="text-xs text-slate-400">{data.kpis.present} / {data.kpis.total_students} recorded</span>
          </div>

          <div className="bg-red-50 rounded-xl p-4 flex flex-col justify-center">
            <div className="flex items-center gap-2 text-red-500 mb-1">
              <UserX className="w-4 h-4" />
              <span className="text-[10px] font-bold uppercase tracking-widest">Absent</span>
            </div>
            <span className="text-2xl font-extrabold text-slate-800">{data.kpis.absent}</span>
            <span className="text-xs text-slate-400">students today</span>
          </div>
          ...
          <div className="bg-amber-50 rounded-xl p-4 flex flex-col justify-center">
            ...
            <span className="text-2xl font-extrabold text-slate-800">{data.kpis.late + data.kpis.excused}</span>
            <span className="text-xs text-slate-400">logged exceptions</span>
          </div>
          ...
          <div className="bg-blue-50 rounded-xl p-4 flex flex-col justify-center">
            ...
            <span className="text-2xl font-extrabold text-slate-800">
              {data.kpis.submitted_classes}<span className="text-sm text-slate-400 font-normal"> / {data.kpis.total_classes}</span>
            </span>
