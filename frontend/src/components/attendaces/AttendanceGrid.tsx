...
export default function AttendanceGrid({ students, exceptions, onStatusChange, onRemarksChange }: GridProps) {
  return (
              ...
              <tr key={student.id} className={`transition-colors ${isException ? 'bg-red-50/50' : 'hover:bg-slate-50'}`}>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center justify-center gap-2 bg-slate-100 p-1 rounded-lg w-max mx-auto">
                    <button onClick={() => onStatusChange(student.id, 'Present')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${currentStatus === 'Present' ? 'bg-white text-emerald-600 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-slate-200'}`}>
                      <CheckCircle2 className="w-4 h-4" /> Present
                    </button>
                    <button onClick={() => onStatusChange(student.id, 'Absent')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${currentStatus === 'Absent' ? 'bg-red-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}>
                      <XCircle className="w-4 h-4" /> Absent
                    </button>
                    <button onClick={() => onStatusChange(student.id, 'Late')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${currentStatus === 'Late' ? 'bg-orange-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}>
                      <Clock className="w-4 h-4" /> Late
                    </button>
                    <button onClick={() => onStatusChange(student.id, 'Excused')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${currentStatus === 'Excused' ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}>
