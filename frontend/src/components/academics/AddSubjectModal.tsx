...
export default function AddSubjectModal({ isOpen, curricula, tiers, departments, onClose, onSuccess }: AddSubjectModalProps) {
  ...
  // Department per checked curriculum — removed as per request, using flat department_id
      ...
      if (subjectId && assignments.length > 0) {
        const results = await Promise.allSettled(
          assignments.flatMap(([curriculumId, tierIds]) => {
            ...
            const department = formData.department_id || null;
            if (tierIds.size === 0) {
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl dark:shadow-none w-full max-w-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 flex items-center justify-between relative overflow-hidden shrink-0">
          <div className="absolute top-0 right-0 -mr-4 -mt-4 w-24 h-24 bg-emerald-100 dark:bg-emerald-500/10 rounded-full opacity-50 pointer-events-none blur-xl"></div>

          <div className="flex items-center gap-3 relative z-10">
            <div className="w-10 h-10 bg-white dark:bg-slate-900 shadow-sm dark:shadow-none border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400">
              <BookOpen className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg leading-tight">Add New Subject</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium tracking-wide">Adds a subject to the master curriculum catalog</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                <input required type="text" placeholder="e.g. MAT101" value={formData.code}
                  className="w-full border border-slate-300 dark:border-slate-600 rounded-xl p-3 outline-none focus:border-emerald-500 dark:focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-400/20 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 transition-all shadow-sm dark:shadow-none"
                  onChange={(e) => setFormData({ ...formData, code: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <label htmlFor="department" className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider block">Default department</label>
                <select id="department" value={formData.department_id} className="w-full border border-slate-300 dark:border-slate-600 rounded-xl p-3 outline-none focus:border-emerald-500 dark:focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-400/20 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 transition-all shadow-sm dark:shadow-none cursor-pointer"
                  onChange={(e) => setFormData({ ...formData, department_id: e.target.value ? Number(e.target.value) : '' })}>
                  <option value="">Uncategorized</option>
                  {(() => {
                    const checkedIds = Object.keys(selectedCurricula).map(Number);
                    const visibleDepts = checkedIds.length > 0
                      ? departments.filter(d => d.is_active && checkedIds.includes(d.curriculum_id))
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500">Filters based on the curricula you select below.</p>
              </div>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider block">Where is this taught? (optional)</label>
            </div>
            <div className="bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-700 rounded-xl divide-y divide-slate-100 dark:divide-slate-700">
              {availableCurricula.length === 0 && (
                <p className="text-xs text-slate-400 dark:text-slate-500 italic px-4 py-3">No curricula configured yet — create one in Curriculum Hub first.</p>
              )}
                ...
                return (
                  <div key={c.id} className="px-4 py-3">
                    <label className="flex items-center gap-3 cursor-pointer">
                      ...
                      <span className="text-slate-700 dark:text-slate-200 text-sm font-semibold">{c.name}</span>
                      <span className="text-xs text-slate-400 dark:text-slate-500 font-mono">{c.code}</span>
                    </label>
                    {checked && curriculumTiers.length > 0 && (
                      <div className="mt-2 ml-7 flex flex-wrap gap-1.5">
                          ...
                          return (
                            <button
                              ...
                              className={`px-2.5 py-1 rounded-full text-xs font-medium border transition ${
                                tierChecked ? 'bg-indigo-100 dark:bg-indigo-500/10 border-indigo-300 dark:border-indigo-500/40 text-indigo-700 dark:text-indigo-400' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-600'
                              }`}
                            >
                              {t.name}
                            </button>
                        ...
                        {selectedTiers.size === 0 && (
                          <span className="text-[11px] text-slate-400 dark:text-slate-500 italic self-center">No tier picked = applies to every tier of {c.code}</span>
                        )}
                      </div>
                    )}
                  </div>
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">Leave everything unchecked to keep it shared across all curricula — you can assign it later from Curriculum Hub.</p>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <label className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider block">Scheduling behavior</label>
            </div>
            <div className="space-y-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-700 rounded-xl p-4">
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={formData.is_core} className="w-4 h-4 text-emerald-600 dark:text-emerald-400 border-slate-300 dark:border-slate-600 rounded focus:ring-emerald-500 dark:focus:ring-emerald-400"
                  onChange={(e) => setFormData({ ...formData, is_core: e.target.checked })} />
                <span className="text-slate-700 dark:text-slate-200 text-sm font-medium">This is a core/mandatory subject</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={formData.allow_double_periods} className="w-4 h-4 text-emerald-600 dark:text-emerald-400 border-slate-300 dark:border-slate-600 rounded focus:ring-emerald-500 dark:focus:ring-emerald-400"
                  onChange={(e) => setFormData({ ...formData, allow_double_periods: e.target.checked })} />
                <span className="text-slate-700 dark:text-slate-200 text-sm font-medium">Allow double periods for this subject</span>
              </label>
              <div className="pt-1 space-y-1.5">
                <label htmlFor="earliest_allowed_time" className="text-xs font-medium text-slate-500 dark:text-slate-400 block">Earliest allowed time (optional)</label>
                <input type="time" id="earliest_allowed_time" value={formData.earliest_allowed_time}
                  className="w-full border border-slate-300 dark:border-slate-600 rounded-xl p-2.5 outline-none focus:border-emerald-500 dark:focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-400/20 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 transition-all shadow-sm dark:shadow-none text-sm"
              ...
              <label className="flex items-center gap-3 cursor-pointer pt-1">
                <input type="checkbox" checked={formData.requires_synchronized_grade_blocking} className="w-4 h-4 text-emerald-600 dark:text-emerald-400 border-slate-300 dark:border-slate-600 rounded focus:ring-emerald-500 dark:focus:ring-emerald-400"
                  onChange={(e) => setFormData({ ...formData, requires_synchronized_grade_blocking: e.target.checked })} />
                <span className="text-slate-700 dark:text-slate-200 text-sm font-medium">Shared/synchronized block subject (e.g. Technical, Agriculture)</span>
              </label>
              {formData.requires_synchronized_grade_blocking && (
                <div className="pl-7 space-y-1.5">
                  <label htmlFor="sync_min_grade" className="text-xs font-medium text-slate-500 dark:text-slate-400 block">Only from this grade level upward (optional)</label>
                  <input type="number" min={1} id="sync_min_grade" value={formData.synchronized_blocking_min_grade}
                    placeholder="e.g. 10"
                    className="w-32 border border-slate-300 dark:border-slate-600 rounded-xl p-2.5 outline-none focus:border-emerald-500 dark:focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-400/20 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 transition-all shadow-sm dark:shadow-none text-sm"
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 flex items-start gap-1.5">
              <GraduationCap className="w-3.5 h-3.5 shrink-0 mt-0.5 text-slate-300 dark:text-slate-600" />
