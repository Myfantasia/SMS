      ...
      <div className="flex bg-white dark:bg-slate-900 rounded-lg p-1 shadow-sm dark:shadow-none border border-slate-100 dark:border-slate-700 w-max">
        {(['Pending', 'Approved', 'Rejected', 'All'] as StatusFilter[]).map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`px-4 py-2 rounded-md text-sm font-medium transition ${filter === s ? 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
          >
            {s}
          </button>
        ))}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-sm text-slate-700 dark:text-slate-200 divide-y divide-slate-50 dark:divide-slate-800">
            {loading ? (
              <tr><td colSpan={7} className="p-8 text-center text-slate-400 dark:text-slate-500">Loading...</td></tr>
            ) : requests.length === 0 ? (
              <tr><td colSpan={7} className="p-8 text-center text-slate-400 dark:text-slate-500">No {filter !== 'All' ? filter.toLowerCase() : ''} pathway requests.</td></tr>
            ) : (
              requests.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <td className="px-6 py-4">
                    <span className="font-semibold text-slate-800 dark:text-slate-100">{r.student_name}</span>
                    <span className="text-xs text-slate-400 dark:text-slate-500 ml-2">{r.student_roll}</span>
                  </td>
                  <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{r.class_name ?? '—'}</td>
                  <td className="px-6 py-4 font-medium text-slate-700 dark:text-slate-200">{r.pathway_name}</td>
                  <td className="px-6 py-4 text-slate-500 dark:text-slate-400">{r.track_name ?? '—'}</td>
                  <td className="px-6 py-4 text-slate-500 dark:text-slate-400 text-xs max-w-[220px]">
                    ...
                    {canDecide && r.status === 'Pending' ? (
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-bold rounded-lg transition-colors border border-emerald-200 dark:border-emerald-500/40 disabled:opacity-50"
                        >
                        ...
                        <button
                          ...
                          disabled={busyId === r.id}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-700 dark:text-red-400 text-xs font-bold rounded-lg transition-colors border border-red-200 dark:border-red-500/40 disabled:opacity-50"
                        >
