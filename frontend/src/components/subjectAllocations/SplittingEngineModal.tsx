...
const SplittingEngineModal: React.FC<SplittingEngineModalProps> = ({ onClose, onSuccess }) => {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 w-full h-full flex flex-col overflow-hidden animate-slideUp">

      {/* 1. PAGE HEADER */}
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex justify-between items-center shrink-0 shadow-sm dark:shadow-none z-10">
        <div className="flex items-center gap-4">
          <button onClick={onClose} className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 transition-colors font-medium text-sm bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 px-3 py-1.5 rounded-lg">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-bold text-slate-800 dark:text-slate-100 text-lg leading-tight">System Configuration Workspace</h2>
              <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">Rules that govern how the allocation algorithm splits electives and balances workload</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-6 shrink-0 flex gap-6">
        <button
          onClick={() => setActiveTab('SPLIT')}
          className={`py-3.5 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeTab === 'SPLIT' ? 'border-indigo-600 text-indigo-700 dark:text-indigo-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
        >
        ...
        <button
          ...
          className={`py-3.5 text-sm font-bold border-b-2 transition-all flex items-center gap-2 ${activeTab === 'GLOBAL' ? 'border-indigo-600 text-indigo-700 dark:text-indigo-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
        >
        ...
        <div className="max-w-7xl mx-auto w-full">
          ...
          {activeTab === 'SPLIT' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-2xl p-5 shadow-sm dark:shadow-none flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  <Info className="w-5 h-5 text-indigo-600 dark:text-indigo-400 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100">Dynamic Balancing Framework</p>
                ...
                <div className="min-w-55 shrink-0">
                  ...
                  <select
                    ...
                    className="w-full border-slate-300 dark:border-slate-600 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 text-sm p-2.5 bg-slate-50 dark:bg-slate-800 border font-bold text-slate-700 dark:text-slate-200 cursor-pointer"
              ...
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <h4 className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider flex items-center gap-2">
                    ...
                    {!isLoadingRules && <span className="text-slate-400 dark:text-slate-500 font-normal normal-case text-xs">&middot; {rules.length} subject{rules.length !== 1 ? 's' : ''}</span>}
                  </h4>
                  <div className="flex items-center gap-2">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="pl-9 pr-8 py-2 w-64 border border-slate-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 outline-none"
                      />
                      {searchQuery && (
                        <button
                ...
                <div className="border border-slate-100 dark:border-slate-700 rounded-2xl overflow-hidden bg-white dark:bg-slate-900 shadow-sm dark:shadow-none">
                  {isLoadingRules ? (
                    <div className="p-10 flex flex-col items-center justify-center gap-3 text-slate-400 dark:text-slate-500">
                      <Loader2 className="w-6 h-6 animate-spin" />
                      <p className="text-sm font-medium">Loading subject thresholds for this grade...</p>
                    </div>
                  ) : filteredRules.length === 0 ? (
                    <div className="p-10 flex flex-col items-center justify-center gap-2 text-slate-400 dark:text-slate-500">
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-slate-50 dark:bg-slate-800/40 border-b border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-700 text-sm text-slate-700 dark:text-slate-200">
                        {filteredRules.map((row) => (
                          <tr key={row.subject_id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/60">
                            <td className="px-5 py-4 font-bold text-slate-800 dark:text-slate-100">{row.subject_name}</td>
                            <td className="px-5 py-4">
                              ...
                              <input
                                ...
                                type="number" min="1"
                                className="w-24 border-slate-300 dark:border-slate-600 rounded-lg p-2 border text-center font-bold bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 outline-none"
                            ...
                            <td className="px-5 py-4">
                              ...
                              <select
                                ...
                                className="border-slate-300 dark:border-slate-600 rounded-lg p-2 border bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 font-medium focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 cursor-pointer"
                            ...
                            <td className="px-5 py-4 text-right">
                              {pendingChanges[row.subject_id] ? (
                                <span className="inline-flex items-center gap-1 text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 rounded-md border border-amber-200 dark:border-amber-500/40">
                                  <AlertTriangle className="w-3.5 h-3.5" /> Unsaved
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 rounded-md border border-emerald-200 dark:border-emerald-500/40">
                ...
                <div className="space-y-3 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {projectedData.map((project, idx) => (
                      <div key={idx} className="border border-slate-100 dark:border-slate-700 rounded-2xl p-5 bg-white dark:bg-slate-900 shadow-sm dark:shadow-none flex flex-col">
                        <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-700 pb-3 mb-3">
                          <div>
                            <h5 className="font-bold text-slate-800 dark:text-slate-100">{project.subject_name}</h5>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Total Pool: <span className="text-indigo-600 dark:text-indigo-400 font-bold">{project.total_enrolled} Candidates</span></p>
                          </div>
                          <span className="text-xs font-mono font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 px-3 py-1 rounded-lg border border-slate-200 dark:border-slate-700">Cap: {project.threshold_limit}</span>
                        </div>
                        <div className="space-y-2">
                          {project.group_distribution.map((grp, gIdx) => {
                            const ratio = Math.min((grp.capacity / project.threshold_limit) * 100, 100);
                            return (
                              <div key={gIdx} className="bg-slate-50 dark:bg-slate-800/40 p-3 rounded-lg border border-slate-100 dark:border-slate-700 text-sm space-y-2">
                                <div className="flex justify-between items-center">
                                  <span className="font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-2"><ChevronRight className="w-4 h-4 text-indigo-500 dark:text-indigo-400" />{grp.name}</span>
                                  <span className="font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-900 px-2 py-1 rounded border border-slate-200 dark:border-slate-700 text-xs">{grp.capacity} Enrolled</span>
                                </div>
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 font-bold rounded-xl transition-all disabled:opacity-40"
                >
              ...
              {showCommitConfirm && (
                <div className="fixed inset-0 z-70 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
                  <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl dark:shadow-none w-full max-w-lg overflow-hidden animate-fade-in">
                    <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-amber-50 dark:bg-amber-500/10 flex items-center gap-2.5">
                      <AlertTriangle className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0" />
                      <h3 className="font-bold text-slate-800 dark:text-slate-100">Confirm Live Commit</h3>
                    </div>
                    <div className="p-6 space-y-4 max-h-100 overflow-y-auto">
                      <div className="grid grid-cols-3 gap-3 text-center text-sm">
                        <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-xl p-3">
                          <p className="text-xl font-black text-emerald-700 dark:text-emerald-400">{streamImpacts.filter(s => s.action === 'created').length}</p>
                          <p className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 uppercase">New Groups</p>
                        </div>
                        <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 rounded-xl p-3">
                          <p className="text-xl font-black text-blue-700 dark:text-blue-400">{streamImpacts.filter(s => s.action === 'updated').length}</p>
                          <p className="text-[11px] font-bold text-blue-600 dark:text-blue-400 uppercase">Capacity Updated</p>
                        </div>
                        <div className="bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 rounded-xl p-3">
                          <p className="text-xl font-black text-red-700 dark:text-red-400">{removedStreams.length}</p>
                          <p className="text-[11px] font-bold text-red-600 dark:text-red-400 uppercase">Groups Removed</p>
                        </div>
                      </div>
                      ...
                      {removedStreams.length > 0 && (
                        <div className="space-y-2">
                          ...
                          {removedStreams.map(r => {
                            ...
                            return (
                              <div key={r.name} className={`p-3 rounded-lg border text-xs ${hasLiveData ? 'bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/40' : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700'}`}>
                                ...
                                {hasLiveData ? (
                                  ...
                                  <p className="text-slate-500 dark:text-slate-400 mt-1">No live data attached — safe to remove.</p>
                                )}
                              </div>
                            );
                          })}
                      ...
                      {removedStreams.length === 0 && (
                        <p className="text-xs text-slate-500 dark:text-slate-400">No existing groups will be removed — only created or capacity-updated in place.</p>
                      )}
                    </div>
          ...
          {activeTab === 'GLOBAL' && (
            ...
            <div className="max-w-6xl space-y-8 animate-fadeIn pb-12">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">Teacher Workload Constraints</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">These variables dictate how the backend assignment algorithm prevents timetable clashes and workload exhaustion.</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {/* Rule Card 1 */}
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 p-6 rounded-2xl shadow-sm dark:shadow-none flex flex-col">
                  <label className="block font-bold text-slate-800 dark:text-slate-100 mb-2">Max Subjects Per Class</label>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 h-8">Limits how many different subjects a single teacher can instruct within the same specific class stream.</p>
                  <input
                    type="number" min="1"
                    aria-label="Max Subjects Per Class"
                    className="w-full border-slate-300 dark:border-slate-600 rounded-lg p-3 font-bold bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 mt-auto"
                ...
                {/* Rule Card 2 */}
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 p-6 rounded-2xl shadow-sm dark:shadow-none flex flex-col">
                  <label className="block font-bold text-slate-800 dark:text-slate-100 mb-2">Max Streams Per Subject</label>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 h-8">Limits how many physical rooms a teacher can be assigned to for an identical subject.</p>
                  <input
                    type="number" min="1"
                ...
                {/* Rule Card 3 */}
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 p-6 rounded-2xl shadow-sm dark:shadow-none flex flex-col">
                  <label className="block font-bold text-slate-800 dark:text-slate-100 mb-2">Absolute Weekly Lesson Cap</label>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 h-8">Limits the total number of teaching periods a teacher can be assigned across all subjects.</p>
                  <input
                    type="number" min="1"
                ...
                {/* Rule Card 4 */}
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 p-6 rounded-2xl shadow-sm dark:shadow-none flex flex-col">
                  <label className="block font-bold text-slate-800 dark:text-slate-100 mb-2">Total Class Group Limit (Prep Cap)</label>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 h-8">Restricts the total number of distinct class sections a teacher manages to control grading overhead.</p>
                  <input
                    type="number" min="1"
                ...
                <div className={`p-6 rounded-2xl shadow-sm dark:shadow-none flex flex-col border transition-colors ${globalPolicy.enforce_prep_consolidation ? 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700' : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700 opacity-80'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <label className="block font-bold text-slate-800 dark:text-slate-100">Minimum Prep Target</label>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 h-8">Sets an optimization goal to bundle assignments, minimizing unique course preparations per teacher.</p>
                  <input
                    type="number" min="1"
                    disabled={!globalPolicy.enforce_prep_consolidation}
                    aria-label="Minimum Prep Target"
                    className="w-full border-slate-300 dark:border-slate-600 rounded-lg p-3 font-bold bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 mt-auto disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-500 disabled:cursor-not-allowed"
              ...
              {/* Cross Grade Setting */}
              <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 p-6 rounded-2xl shadow-sm dark:shadow-none flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-slate-800 dark:text-slate-100">Allow Cross-Grade Teaching</h4>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-xl">If enabled, the algorithm permits a teacher to instruct the same subject across different grade levels (e.g., Grade 7 & Grade 8 Mathematics).</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer shrink-0">
                  <input
              ...
              <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/40 p-6 rounded-2xl shadow-sm dark:shadow-none">
                <h4 className="font-bold text-amber-900 dark:text-amber-300 mb-4 flex items-center gap-2">
                  <AlertTriangle className="w-5 h-5" /> Enforcement Compliance Mode
                </h4>
                <div className="flex flex-col sm:flex-row gap-4">
                  <button
                    onClick={() => handlePolicyChange('enforcement_mode', 'STRICT')}
                    className={`flex-1 p-4 rounded-xl border-2 text-left transition-all ${globalPolicy.enforcement_mode === 'STRICT' ? 'bg-white dark:bg-slate-900 border-amber-500 shadow-md dark:shadow-none' : 'bg-amber-50/50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/40 hover:bg-white dark:hover:bg-slate-900'}`}
                  >
                    <div className="font-bold text-amber-900 dark:text-amber-300 mb-1">STRICT: Hard Blocking</div>
                    <div className="text-xs text-amber-700/80 dark:text-amber-400/80">The system completely rejects saving if any workload rule is violated. Standard protocol.</div>
                  </button>
                  <button
                    onClick={() => handlePolicyChange('enforcement_mode', 'SOFT')}
                    className={`flex-1 p-4 rounded-xl border-2 text-left transition-all ${globalPolicy.enforcement_mode === 'SOFT' ? 'bg-white dark:bg-slate-900 border-amber-500 shadow-md dark:shadow-none' : 'bg-amber-50/50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/40 hover:bg-white dark:hover:bg-slate-900'}`}
                  >
                    <div className="font-bold text-amber-900 dark:text-amber-300 mb-1">SOFT: Override Warnings</div>
                    <div className="text-xs text-amber-700/80 dark:text-amber-400/80">Allows the administrator to force-save matrix configurations while logging amber warnings.</div>
                  </button>
                </div>
              </div>
